...
int Linear_Conflict(int k, int** mat){
    ...
    vector <pair<int,int>> column_num;
    ...
    for(int i = 0; i < k; i++){
        for(int j = 0; j < k; j++){
            if(mat[i][j] != 0 && row_num[mat[i][j]-1].second == i){
                for(int m = j+1; m < k; m++){
                    if(mat[i][m] != 0 && row_num[mat[i][m]-1].second == i && mat[i][j] > mat[i][m]){
                        linear_conlict++;
                    }
                }
            }
...
int CalculateCost(int func, int k, int** initial, int** goal, int move_cost){
    ...
    if(func == 3) {
        ...
        //cout << "lin cost " << lin_cost << "man cost " << heu_cost_man << " move cost " << move_cost << endl;
...
void PuzzleSolve(int method, int **initial, int k, int x, int y, int **goal) {
    ...
    while (!puzzleboardopen.empty()) {
        ...
        puzzleboardopen.pop();
        if (!isSame(puzzleboardclosed, current, k)){
